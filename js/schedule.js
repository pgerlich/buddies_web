...
angular.module("myApp").controller("mainCtrl", function($scope, $uibModal){
    ...
    function getWashesForUser(){
        ...
        query.find({
            success: function(results) {
                ...
                for(var i = 0; i < results.length; i++ ) {
                    var wash = results[i];
                    var job = {date: wash.get("date"), time: wash.get("time"), status: convertStatusToString(wash.get("status")), washId: wash.get("washId"), employeeNotes: wash.get("employeeNotes"), type: wash.get("vehicle").get("type"), pointer: wash};
    ...
    $scope.createJobTransaction = function(job){
        ...
        var ccid = job.card;
        var transactionURL = "php/payForWash.php?TIPAMT="+tip+"&BASEAMT="+baseCost+"&CID="+cid+"&AID="+aid+"&CCID="+ccid;
...
angular.module('myApp').controller('jobCompletionCtrl', function ($scope, $uibModalInstance, job) {
    ...
    $scope.getCardsForUser = function(){
        $.get( "php/retrieveCustomer.php?CID=" + Parse.User.current().get("stripeAccount"), function( data ) {
